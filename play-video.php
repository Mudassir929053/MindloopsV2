...
<body class="bg-white text-dark">
    ...
    $uid = $_SESSION['uid'];
    $sql_view = "INSERT INTO video_view (video_id,viewer_id) VALUES ('$video_id','$uid') ON DUPLICATE KEY UPDATE view_date = '$currentDateTime',status=0";
//   exit;
        ...
        <main class="main-content">
            ...
            <div class="container ">
                <div class="row">
                    ...
                    <div class="col-lg-8 mt-4 bg-white">
                        <?php
        ...
        WHERE video_id = $video_id AND vld_disliked_by IS NOT NULL
                                    ...
                                    <div class="d-flex flex-column flex-md-row align-items-center">
                                        ...
                                        if ($result) {
                                            ...
                                            if ($row) {
                                                ...
                                                echo '<img src="' . $avatar . '" class="img-fluid rounded-circle mb-3 mb-md-0" width="60" alt="Profile Picture">';
                                            }
                                            else{
                        ...
                        <div class="col-md-12">
                            <div class="row py-4">
                                ...
                                <?php
                                $uid = $_SESSION['uid'];
                                $query = "SELECT vc.vc_id, vc.video_id, vc.vc_content, vc.created_by, vc.approved_vc, u.profile_pic, u.full_name,u.username,u.email FROM video_comment vc JOIN user_table u ON vc.created_by = u.uid WHERE vc.video_id = $video_id  ORDER BY vc.created_date DESC";
                                $result = $mysqli->query($query);
                                if ($result) {
                                    if ($result->num_rows > 0) {
                                        while ($row = $result->fetch_assoc()) {
                                            ...
                                            if (strpos($row['profile_pic'], 'http') !== false) {
                                                ...
                                                $img_url = "assets/uploads/avatar/" . $row['profile_pic'];
                                            }
                                            echo '<div class="d-flex align-items-center position-relative">
                                                 <img src="'.$img_url.'" alt="Profile Picture" class="rounded-circle" width="50" height="50">
                                                 <div class="mx-3 align-self-center">' . $user_name . '</div>';
                    ...
                    <div class="col-lg-4 col-md-12 mt-4">
                        ...
                        <div class="card mb-3 border-0 mb-5 py-3 bg-white">
                            <div class="row g-0">
                                <?php
                                $excludeCategoryID = isset($_GET['category_id']) ? $_GET['category_id'] : null;
                                $sql = "SELECT v.`video_id`, v.`video_title`, v.`video_thumbnail`, v.`video_description`, v.`video_category_id`, v.`video_url`, v.`video_audience_id`, v.`created_date`,
                                 ...
                                 LEFT JOIN `video_categories` c ON v.`video_category_id` = c.`category_id`
                                 LEFT JOIN `user_role` ur ON v.`video_audience_id` = ur.`id` WHERE v.`video_category_id` = '$category_id'";
                                $result = $mysqli->query($sql);
                                $result = $mysqli->query($sql);
                                ...
                                ?>
                                        <div class="col-md-5 mb-4 zooms <?php if ($counter > $displayLimit) echo 'hidden'; ?>">
                                            ...
                                            <a href="play-video.php?id=<?= $row['video_id'] ?>&category_id=<?= $category_id; ?>">
                            ...
                            <div class="col-md-6 col-lg-4 py-3 <?php if ($counter > $displayLimit) echo 'hidden'; ?>">
                                <div class="card border-0 bg-white text-dark">
                                    ...
                                    <div class="ratio ratio-16x9" style="height: 310px;"> <!-- Adjust the height value as needed -->
                                        <a href="play-video.php?id=<?= $row['video_id'] ?>&category_id=<?= $category_id; ?>" class="zooms">
